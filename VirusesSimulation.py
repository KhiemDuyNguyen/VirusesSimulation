...
class Patient(object):
    ...
    def update(self):
        ...
        popDen = len(self.viruses)/self.maxPop
        
        for virus in self.viruses[:]:
            try:
                child = virus.reproduce(popDen)
    ...
    #TESTING
#viruses = [
#SimpleVirus(0.76, 0.93),
#SimpleVirus(0.12, 0.96),
#SimpleVirus(0.52, 0.98),
#SimpleVirus(0.06, 0.88),
#SimpleVirus(0.29, 0.88)
#]
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        ...
        newPatient = Patient(virusesList, maxPop)
        for update in range(300):
            newPatient.update()
            popAverage[update] += newPatient.getTotalPop()
    for i in range (300):
        popAverage[i] = popAverage[i]/numTrials

    pylab.plot(popAverage, label = "SimpleVirus")
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if (random.random() <= self.maxBirthProb * (1 - popDensity) and (isResist)):
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in self.viruses[:]:
            try:
                child = virus.reproduce(popDen, self.prescription)
            except NoChildException:
                pass
            else:
                self.viruses.append(child)
...
#virus1 = ResistantVirus(1.0, 0.0, {"drug1": False}, 0.0)
#virus2 = ResistantVirus(1.0, 0.0, {"drug1": True}, 0.0)
#virus3 = ResistantVirus(1.0, 0.0, {"drug2": True}, 0.0)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    """
    popAverage = []
    popResistanceAverage = []
    for i in range (300):
        popAverage.append(0) 
        popResistanceAverage.append(0)
    
    for trial in range(numTrials):
        virusesList = []
        for virus in range(numViruses):
            virusesList.append(ResistantVirus(maxBirthProb,clearProb, resistances, mutProb))
        newPatient = TreatedPatient(virusesList, maxPop)
        for update in range(150):
            newPatient.update()
            popAverage[update] += newPatient.getTotalPop()
            popResistanceAverage[update] += newPatient.getResistPop(list(resistances.keys()))
        newPatient.addPrescription('guttagonol')
        for update in range(150,300):
            newPatient.update()
            popAverage[update] += newPatient.getTotalPop()
            popResistanceAverage[update] += newPatient.getResistPop(list(resistances.keys()))
    ...
    for i in range (300):
        popAverage[i] = popAverage[i]/numTrials
        popResistanceAverage[i] = popResistanceAverage[i]/numTrials
        
    pylab.plot(popAverage, label = "VirusesPop")
    pylab.plot(popResistanceAverage, label = "ResistantVirusPop")
    pylab.title("SimpleVirus simulation")
    pylab.xlabel("Time Steps")
    pylab.ylabel("Average Virus Population")
    pylab.legend(loc = "best")
